...
interface IArborAudioCollections {
...
// dev
    ...
    function getHighestBidderDev(
        ...
        // IArborAudioCollections(nft).buyDev(payable(highestBidder), 1);
    }


    function bidDev(bytes32 blindedBid, string calldata amount, string calldata secret) external payable {
        // require deposits[msg.sender] doesn't already exist
        // require that deposit meets auction deposit requirement
        require(bids.length < MAX_BIDS, "max bids reached");
        require(msg.value == DEPOSIT_AMOUNT, "deposit must be 1 ether");
    ...
    function getBidsDev()
        ...
        view
        onlyAfter(biddingEnd)
        returns (bytes32[] memory, string[] memory, string[] memory)
    {
        require(amounts.length == 4);
        return (bids, amounts, secrets);
    }



    function getData() public view returns (address, address , address , uint256) {
        return (beneficiary, auctioneer, highestBidder, highestBid);
    }
}
